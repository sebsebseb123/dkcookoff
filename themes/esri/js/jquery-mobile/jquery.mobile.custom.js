...
$.widget( "mobile.dialog", $.mobile.widget, {
	options: {
		closeBtnText 	: "Close",
		selallBtnText	: "All/Clear",
		overlayTheme	: "a",
		initSelector	: ":jqmData(role='dialog')"
	},
	_create: function() {
		//Language Text
		var selallText = 'All/Clear';

		var currentLang = $('html').attr('lang');
		if (currentLang == 'fr') selallText = 'Tous/Clairs';
      ...
      headerSelAll;
		...
		if( widget.isMultiple ) {
			...
			headerSelAll = $( "<a>", {
				...
				"class": "ui-btn-right"
			}).attr( "data-corners", "false" ).addClass( "ui-header-all" ).appendTo( header ).buttonMarkup();
		}
		...
		$.extend( widget, {
			...
			build: function() {
				...
				if( self.isMultiple ){
					...
					self.headerSelAll.click(function() {
						//Overlay or Not
						var currentState = $(this).parent('div').attr('data-role');

						//Button is in Overlay
						if (currentState == 'header') {
							var currentId = $(this).closest('.ui-dialog-contain').find('ul').attr('id')

						//Button is Not in Overlay Mode
						} else {
							var currentId = $(this).closest('.ui-selectmenu').children('ul').attr('id');
						}
						
						currentId = currentId.replace('-menu', '');
						currentAll = true;

						$.each($('#' + currentId).children('option:not([data-placeholder="true"], [value="All"])'), function() {
				          	if ($(this).attr('selected')==undefined) currentAll = false;
				        });

						if (currentAll) {
							//Deselect All
							$('#' + currentId).children('option:not([data-placeholder="true"])').removeAttr('selected');
						} else {
							//Select All
							$('#' + currentId).children('option:not([data-placeholder="true"])').attr('selected', 'selected');
						}
