<?php
	...
	function moxiecode_variable_get( $name, $default = NULL )
	{
		global $conf;
	...
	$prevWwwRoot = '';
	$prevUrlPrefix = "{proto}://{host}/";
	$fileRootPath = '../../../../../../sites/default/files/filemanager';
		
	// NOTE: Enable the following if the above fails
	// $prevWwwRoot depending on Acquia host env.
	// The below given $prevWwwRoot and $fileRootPath are for downsview. Change them for ESRI.
	/*
	switch ($_ENV['AH_SITE_ENVIRONMENT']) {
	  case 'prod':
      case 'test':
      case 'dev':
        $prevWwwRoot = '/vol/ebs1/gfs/majorisdev'; // Symlink pointing to webroot. Change this to match ESRI settings. This is for downsview.
		$prevUrlPrefix = 'http://esricanadadev.prod.acquia-sites.com/';
		$fileRootPath = '/mnt/files/majorisdev/sites/default/files/filemanager'; // Change this to match ESRI settings. This is for downsview.
        break;
      default:
        $prevWwwRoot = '';
	    $prevUrlPrefix = "{proto}://{host}/";
	...
	*/
	...
	// Preview options
	$mcFileManagerConfig['preview.wwwroot'] = $prevWwwRoot; // absolute or relative from this script path (c:/Inetpub/wwwroot).
	...
	$mcFileManagerConfig['filesystem.rootpath'] = $fileRootPath; // absolute or relative from this script path, required.
	$mcFileManagerConfig['filesystem.path'] = $fileRootPath; // absolute or relative from this script path.
